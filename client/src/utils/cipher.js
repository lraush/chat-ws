...
/** @param {number} rotations number of Caesar steps (0 = plain text) */
export function encryptMessage(plain, rotations) {
  const steps = normalizeRotations(rotations);
  if (steps === 0 || !plain) return plain;
  return applyRotations(plain, steps, true);
}

export function decryptMessage(cipher, rotations) {
  const steps = normalizeRotations(rotations);
  if (steps === 0 || !cipher) return cipher;
  return applyRotations(cipher, steps, false);
}

export function normalizeRotations(value) {
